...
RE_DATE_3 = re.compile(r'(\d{2}[./-]){2}\d{4}')
"""
Пояснения по паттерну r'(\d{2}[./-]){2}\d{4}' - почему findall возвращает вторую часть, а не первую?
    Скобки отвечают за группировку значений и указание, что будет возвращено. В данном случае в составе
    паттерного выражения присутствует удвоение искомого шаблона '{2}', помещенного за скобками.
    Соответственно в зависимости от требуемого результата, при необходимости получить только первую часть
    даты, правильнее переписать выражение следующим образом '(\d{2}[./-])\d{2}[./-]\d{4}', либо
    воспользоваться группировками '?:' как выполняли выше.
"""
print()
